-- Adds coupon capture to create_inquiry.
--
-- Everything else is byte-identical to 20260807120000_create_inquiry_email_only.sql.
...
--
-- The function does not decide the discount. It stores what the service computed, the
-- same way it already stores subtotal_cents — Postgres has no view of the coupon table
...
comment on function public.create_inquiry is
  'Atomically creates one order, its items, and one pending notification_log row for the email channel. Stores the coupon code and discount the application computed; it does not evaluate coupons itself. Idempotent on idempotency_key: a replay returns the original order with created = false.';
